...
from .birdeye_render import BirdeyeRender, RoutePlanner
from .helpers import (
...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    # Destination
    if params['task_mode'] == 'random':
      self.dests = [[-3.68, 219.41, 0.5], [-7.530, 158.980, 0.5], [193.780, 266.430, 0.5], [193.780, 218.790, 0.50],
                    [189.930, 160.580, 0.50], [131.69003906, 105.5499707, 0.50], [173.87005859,109.40003906, 0.50],
                    [55.4100293, 109.39999023, 0.50], [136.11003906, 302.56996094, 0.50], [88.83001953, 306.420, 0.50],
                    [118.77003906, 237.42996094, 0.50], [162.76005859, 241.28003906, 0.50], [88.71001953, 241.280, 0.50]] # x y z
    else:
      self.dests = None
    ...
    client = carla.Client('192.168.100.101', params['port'])
...
#     world.load_map_layer(carla.MapLayer.Buildings)
    ...
    self.lidar_bp.set_attribute('range', '10')
    ...
    self.camera_trans = carla.Transform(carla.Location(x=3.2, z=4))
...
#     for spawn_point in init_transforms:
#       print(spawn_point.location.x, spawn_point.location.y, spawn_point.location.z, spawn_point.rotation.pitch, spawn_point.rotation.yaw, spawn_point.rotation.roll)
    ...
    while True:
      ...
      if self.task_mode == 'random':
        #self.start=[-3.68 ,288.22,-90.0] # static x y yaw
        #transform = set_carla_transform(self.start) #return transform sets xyz pitch yaw roll
...
#       point_cloud.append([point.x, point.y, -point.z])
    point_cloud = np.frombuffer(self.lidar_data.raw_data, dtype=np.dtype('f4'))
    point_cloud = np.reshape(point_cloud, (int(point_cloud.shape[0] / 4), 4))
    point_cloud2 = point_cloud[:, :3]
    ...
    lidar, _ = np.histogramdd(point_cloud2, bins=(x_bins, y_bins, z_bins))
